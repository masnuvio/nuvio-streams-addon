...
const rogBase = "https://rogmovies.store";
...
async function getStreams(type, imdbId, season, episode) {
    try {
        ...
        console.log(`[Rogmovies] Searching for: ${searchQuery}`);

        const searchUrl = `${rogBase}/page/1/?s=${encodeURIComponent(searchQuery)}`;
        ...
        if (!match) {
            console.log('[Rogmovies] No matching results found.');
            return [];
        }

        console.log(`[Rogmovies] Found match: ${match.title}`);
        ...
        console.error(`[Rogmovies] Error: ${e.message}`);
        return [];
    }
}
...
async function getMetadata(type, id) {
    try {
        ...
        console.error(`[Rogmovies] Metadata error: ${e.message}`);
    }
    return null;
}
