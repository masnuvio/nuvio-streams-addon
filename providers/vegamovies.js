...
const vegaBase = "https://m.vegamovies.cricket";
...
async function getStreams(type, imdbId, season, episode) {
    try {
        ...
        console.log(`[VegaMovies] Searching for: ${searchQuery}`);

        const searchUrl = `${vegaBase}/page/1/?s=${encodeURIComponent(searchQuery)}`;
        ...
        if (!match) {
            console.log('[VegaMovies] No matching results found.');
            return [];
        }

        console.log(`[VegaMovies] Found match: ${match.title}`);
            ...
            for (let i = 0; i < hTags.length; i++) {
                ...
                if (realSeason === parseInt(season)) {
                    ...
                    if (targetLink) {
                        ...
                        // Extract vcloud links
                        $$$('p > a').each((k, el) => {
                            const href = $$$(el).attr('href');
                            if (href && href.includes('vcloud')) {
                                // We need to map this to an episode
                                // Heuristic: index + 1 or text parsing
                                // The Kotlin code maps by index: val key = Pair(realSeason, vcloudLinks.indexOf(vcloudlink) + 1)
                                // But we are iterating.
                                // Let's assume the order matches episodes.
                                // We need to find WHICH episode this link corresponds to.
                                // If we are looking for a specific episode, we can try to find it.

                                // Since we don't have the full list here easily without fetching all, 
                                // we might need to fetch all and filter.
                                // For now, let's just push it if it looks like a vcloud link and hope the user can select.
                                // OR, better: The Kotlin code iterates all vcloud links and maps them.
                                // We should do the same.

                                // Since we are in a loop of hTags (qualities), we can add streams for this quality.
                                // But we need to know the episode number.
                                // The Kotlin code assumes the order of links matches episode numbers (1, 2, 3...).
                                // So if we want episode E, we pick the (E-1)th link.

                                // Let's collect all vcloud links first
                            }
                        });
...
async function getMetadata(type, id) {
    try {
        ...
        console.error(`[VegaMovies] Metadata error: ${e.message}`);
    }
    return null;
}
